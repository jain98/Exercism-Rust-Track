...
fn test_empty_is_a_sublist_of_anything() {
    fn adder(v: Vec<u32>) -> u32 {
        let n = 4;
        let chunk = v.len()/n;
        let mut spawned: Vec<thread::JoinHandle<u32>> = vec![];
        let arc = Arc::new(v);

        for i in 0..n {
            let cloned = Arc::clone(&arc);
            spawned.push(thread::spawn(move || {
                cloned[i*chunk..(i*chunk+chunk)].iter().sum()
            }));
        }

        spawned.iter().map(|t| t.join().unwrap()).sum()
    }
